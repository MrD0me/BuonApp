...
async function main() {
  ...
  db.prepare(`INSERT INTO users (id, name, username, password, role, is_active, station_assignments_configured, created_at, updated_at)
    ...
    unrestrictedChefId, 'kds-contract-unrestricted', bcrypt.hashSync('testpass123', 10), now(), now(),
  );
  ...
  db.prepare(`INSERT INTO users (id, name, username, password, role, category_ids, is_active, created_at, updated_at)
    VALUES (?, 'Restricted Chef', ?, ?, 'chef', ?, 1, ?, ?)`).run(
    chefId, 'kds-contract-chef', bcrypt.hashSync('testpass123', 10), JSON.stringify(['kds-contract-category']), now(), now(),
  );
  ...
  db.prepare(`INSERT INTO users (id, name, username, password, role, is_active, created_at, updated_at)
    ...
    inactiveChefId, 'kds-contract-inactive', bcrypt.hashSync('testpass123', 10), now(), now(),
  );
