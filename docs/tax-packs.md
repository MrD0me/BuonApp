...
BuonApp's tax system is split in two, per [`docs/tax-engine-v2-spec.md`](tax-engine-v2-spec.md):
...
- Effective dates, publisher, minimum compatible BuonApp version
...
2. Give it a unique `id` using the lowercase, hyphenated full country name (for example, `official-india`, `official-thailand`, or `official-united-states`). Keep `country` as its ISO alpha-2 code (for example, `IN`, `TH`, or `US`), because BuonApp uses that field to match the store. Set `publisher` to your name/org (anything other than `local` — `local` is reserved for the in-app manual/unbundled pack and can never be published), and fill in `jurisdiction`, `currency`, `taxRounding`, `payableRounding`.
...
4. Add the file to `main/tax-packs/` in this repo (not a new repo — see "Where packs live" below) and open a PR. Only the generic/manual no-tax pack is bundled with and auto-activated by a new installation. Every official country pack is catalog-only: an owner explicitly enables the matching pack from Settings → Tax Configuration, where BuonApp downloads, verifies, installs, and activates it.
...
1. Downloads the previous cumulative `catalog.json` (if one exists) from the `BuonApp-Plugins` releases.
