...
BuonApp provides an offline-first, Git-native internationalization architecture built on [`use-intl`](https://www.npmjs.com/package/use-intl) v4. Translations are managed as static JSON files committed directly to the repository, requiring zero external software-as-a-service (SaaS) platforms, API tokens, or network connectivity.
...
5. **Decoupled domains:** UI language and tenant regional settings (store timezone and currency) are decoupled domains:
   ...
   Changing the UI language never alters store currency formatting.
...
BuonApp uses ICU MessageFormat strings for plurals and variable interpolation:
...
Adding a new language to BuonApp is a standardized 6-step workflow.
...
BuonApp natively supports bidirectional and RTL languages (such as Persian / Farsi `fa`, Arabic `ar`, Hebrew `he`, Urdu `ur`).
...
BuonApp includes automated RTL verification test suites:
```sh
...
BuonApp enforces two complementary rules for language quality:
...
## UI language vs. country profiles

UI language translation is strictly decoupled from business country profiles:
...
- **Country Profiles:** Located in `main/countries.ts`, controlling currency symbols, locale defaults, and the label used for the business registration number. Store timezone remains tenant-authoritative; calendar mode, digit representation, and currency display are constrained by the active country's `localeOptions`.

Translating BuonApp into Japanese or German does not require touching a country profile, and adding a country profile does not require adding a new UI language.
...
| `npm test` | Run full BuonApp test suite |
